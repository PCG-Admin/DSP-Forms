...
export function DieselCartTrailerForm({ brand }: DieselCartTrailerFormProps) {
  ...
  useEffect(() => {
    const fetchNextNumber = async () => {
      try {
        const res = await fetch('/api/next-document?formType=diesel-cart-trailer')
        if (res.ok) {
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="space-y-2">
            ...
            <Input id="registrationNumber" value={formData.registrationNumber} onChange={e => setFormData(p => ({ ...p, registrationNumber: e.target.value }))} placeholder="e.g. DC-001" required />
          </div>
        </CardContent>
      </Card>
