...
// ============================================================================
// INSPECTION ITEMS – as they appear in the Diesel Cart Trailer PDF
// ============================================================================
...
export function DieselCartTrailerForm({ brand }: DieselCartTrailerFormProps) {
  ...
  const documentNo = useMemo(() => {
    const d = new Date()
    return `DC-${d.getFullYear().toString().slice(-2)}${(d.getMonth()+1).toString().padStart(2,"0")}${d.getDate().toString().padStart(2,"0")}-${Math.floor(Math.random()*1000).toString().padStart(3,"0")}`
  }, [])
    ...
    <form onSubmit={handleSubmit} className="mx-auto max-w-3xl space-y-6 p-4 pb-12 lg:p-8 lg:pb-16">
      ...
      <Card>
        <CardHeader className="text-center">
          ...
          <CardDescription>Document Ref: HSEMS/8.1.9/REG/014 | Rev. 2 | 27.03.2020</CardDescription>
        </CardHeader>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="space-y-2"><Label htmlFor="registrationNumber">Registration number <span className="text-destructive">*</span></Label><Input id="registrationNumber" value={formData.registrationNumber} onChange={e => setFormData(p=>({...p, registrationNumber:e.target.value}))} placeholder="e.g. DC-001" required /></div>
        </CardContent>
      </Card>
      ...
      <div className="grid grid-cols-4 gap-2 text-xs text-muted-foreground border-t pt-4">
        <div><span className="font-semibold">Document Reference No.</span><br />HSEMS / 8.1.9 / REG / 014</div>
