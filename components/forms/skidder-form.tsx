...
const itemIconMap: Record<string, string> = {
  ...
  "Winch": "winch.png",
  "Tackle": "boom-structure.png", // fallback
...
export function SkidderForm() {
  ...
  const documentNo = useMemo(() => {
    const d = new Date()
    return `SK-${d.getFullYear().toString().slice(-2)}${(d.getMonth()+1).toString().padStart(2,"0")}${d.getDate().toString().padStart(2,"0")}-${Math.floor(Math.random()*1000).toString().padStart(3,"0")}`
  }, [])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!formData.operatorName || !formData.machineNumber) { toast.error("Please fill in all required fields"); return }
    ...
    <form onSubmit={handleSubmit} className="mx-auto max-w-3xl space-y-6 p-4 pb-12 lg:p-8 lg:pb-16">
      ...
      <Card>
        <CardHeader className="text-center">
          ...
          <CardDescription>Document Ref: HSEMS/8.1.19/REG/006 | Rev. 2 | 27.03.2020</CardDescription>
        </CardHeader>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 sm:grid-cols-2">
          <div className="space-y-2"><Label htmlFor="operatorName">Operators name and surname <span className="text-destructive">*</span></Label><Input id="operatorName" value={formData.operatorName} onChange={e => setFormData(p=>({...p, operatorName:e.target.value}))} placeholder="Enter operator name" required /></div>
          ...
          <div className="space-y-2"><Label htmlFor="machineNumber">Machine number <span className="text-destructive">*</span></Label><Input id="machineNumber" value={formData.machineNumber} onChange={e => setFormData(p=>({...p, machineNumber:e.target.value}))} placeholder="e.g. SK-001" required /></div>
      ...
      <div className="grid grid-cols-4 gap-2 text-xs text-muted-foreground border-t pt-4">
        <div><span className="font-semibold">Document Reference No.</span><br />HSEMS / 8.1.19 / REG / 006</div>
