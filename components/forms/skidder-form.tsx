...
export function SkidderForm({ brand }: SkidderFormProps) {
  ...
  useEffect(() => {
    const fetchNextNumber = async () => {
      try {
        const res = await fetch('/api/next-document?formType=skidder')
        if (res.ok) {
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 sm:grid-cols-2">
          ...
          <NameSelector
            ...
            label="Operators name and surname"
            required
          ...
          <div className="space-y-2">
            <Label htmlFor="machineNumber">Machine number <span className="text-destructive">*</span></Label>
            <Input id="machineNumber" value={formData.machineNumber} onChange={e => setFormData(p => ({ ...p, machineNumber: e.target.value }))} placeholder="e.g. SK-001" required />
          </div>
