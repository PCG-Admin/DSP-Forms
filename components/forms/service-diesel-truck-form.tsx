  ...
  return (
    <div className="flex flex-col sm:grid sm:grid-cols-[1fr_auto_auto] sm:items-center border-b border-gray-200 last:border-0">
      ...
      <div className="py-2 px-3 sm:py-3 sm:px-4 bg-gray-50 sm:border-r sm:border-gray-200 flex justify-center sm:w-[150px]">
        <Image src={iconSrc} alt={item} width={150} height={150} className="object-contain h-20 w-auto sm:h-[150px] sm:w-[150px]" />
      </div>
      <div className="py-2 px-3 sm:py-3 sm:pl-4 flex items-center gap-2 w-full sm:w-auto">
        <Button type="button" variant={isSelected("ok") ? "default" : "outline"} size="sm"
          ...
          className={`flex-1 sm:flex-none sm:w-16 ${isSelected("ok") ? "bg-green-600 hover:bg-green-700" : ""}`}>OK</Button>
        <Button type="button" variant={isSelected("def") ? "default" : "outline"} size="sm"
          onClick={() => onChange(isSelected("def") ? null : "def")}
          className={`flex-1 sm:flex-none sm:w-16 ${isSelected("def") ? "bg-red-600 hover:bg-red-700" : ""}`}>DEF</Button>
        <Button type="button" variant={isSelected("na") ? "default" : "outline"} size="sm"
          onClick={() => onChange(isSelected("na") ? null : "na")}
          className={`flex-1 sm:flex-none sm:w-16 ${isSelected("na") ? "bg-gray-600 hover:bg-gray-700" : ""}`}>N/A</Button>
      </div>
    </div>
  )
}
...
export function ServiceDieselTruckForm({ brand }: ServiceDieselTruckFormProps) {
  ...
  useEffect(() => {
    const fetchNextNumber = async () => {
      try {
        const res = await fetch('/api/next-document?formType=service-diesel-truck')
        if (res.ok) {
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="space-y-2">
            ...
            <Input id="odometerStartHours" type="number" value={formData.odometerStartHours} onChange={e => setFormData(p => ({ ...p, odometerStartHours: e.target.value }))} placeholder="e.g. 1250" />
          </div>

          <div className="space-y-2">
            <Label htmlFor="odometerStopHours">Odometer stop – hours</Label>
            <Input id="odometerStopHours" type="number" value={formData.odometerStopHours} onChange={e => setFormData(p => ({ ...p, odometerStopHours: e.target.value }))} placeholder="e.g. 1262" />
          </div>

          <div className="space-y-2">
            <Label htmlFor="odometerStartKm">Odometer start – kilometres</Label>
            <Input id="odometerStartKm" type="number" value={formData.odometerStartKm} onChange={e => setFormData(p => ({ ...p, odometerStartKm: e.target.value }))} placeholder="e.g. 45200" />
          </div>

          <div className="space-y-2">
            <Label htmlFor="odometerStopKm">Odometer stop – kilometres</Label>
            <Input id="odometerStopKm" type="number" value={formData.odometerStopKm} onChange={e => setFormData(p => ({ ...p, odometerStopKm: e.target.value }))} placeholder="e.g. 45350" />
          </div>
          ...
          <div className="space-y-2">
            ...
            <Input id="dangerousGoodsCard" type="text" value={formData.dangerousGoodsCard} onChange={e => setFormData(p => ({ ...p, dangerousGoodsCard: e.target.value }))} placeholder="e.g. DG-12345" />
          </div>
        </CardContent>
      </Card>
