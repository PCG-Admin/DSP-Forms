...
export function SelfLoadingForwarderForm({ brand }: SelfLoadingForwarderFormProps) {
  ...
  useEffect(() => {
    const fetchNextNumber = async () => {
      try {
        const res = await fetch('/api/next-document?formType=self-loading-forwarder')
        if (res.ok) {
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="space-y-2">
            ...
            <Input id="unitNumber" value={formData.unitNumber} onChange={e => setFormData(p => ({ ...p, unitNumber: e.target.value }))} placeholder="e.g. SF-001" required />
          </div>
        </CardContent>
      </Card>
