...
export function SelfLoadingForwarderForm() {
  ...
  const documentNo = useMemo(() => {
    const d = new Date()
    return `SF-${d.getFullYear().toString().slice(-2)}${(d.getMonth()+1).toString().padStart(2,"0")}${d.getDate().toString().padStart(2,"0")}-${Math.floor(Math.random()*1000).toString().padStart(3,"0")}`
  }, [])
    ...
    <form onSubmit={handleSubmit} className="mx-auto max-w-3xl space-y-6 p-4 pb-12 lg:p-8 lg:pb-16">
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="space-y-2"><Label htmlFor="unitNumber">Unit number <span className="text-destructive">*</span></Label><Input id="unitNumber" value={formData.unitNumber} onChange={e => setFormData(p=>({...p, unitNumber:e.target.value}))} placeholder="e.g. SF-001" required /></div>
        </CardContent>
      </Card>
      ...
      <div className="grid grid-cols-4 gap-2 text-xs text-muted-foreground border-t pt-4">
        <div><span className="font-semibold">Document Reference No.</span><br />HSEMS / 8.1.19 / REG / 003</div>
